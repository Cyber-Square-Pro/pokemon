...
abstract class _PokemonStoreBase with Store {
  ...
  @computed
  int get index => pokemonsSummary!.indexWhere((it) => it.number == _pokemon!.number);

  @computed
  PokemonFilter get pokemonFilter => _pokemonFilter;

  @computed
  List<PokemonSummary> get favoritesPokemonsSummary => _favoritesPokemonsSummary;

  @action
  Future<void> setPokemon(int index) async {
    _pokemonSummary = pokemonsSummary![index];

    final pokemonDetailsIndex = _pokemons.indexWhere((it) => it.number == _pokemonSummary!.number);

    if (pokemonDetailsIndex < 0) {
      final fetchedPokemon = await _pokemonRepository.fetchPokemon(_pokemonSummary!.number);
  ...
  void addFavoritePokemon(String number) {
    final indexFavorite = _favoritesPokemonsSummary.indexWhere((it) => it.number == number);

    final indexAll = _pokemonsSummary!.indexWhere((it) => it.number == number);

    if (indexFavorite < 0 && indexAll >= 0) {
      _favoritesPokemonsSummary =
          ObservableList.of([..._favoritesPokemonsSummary, _pokemonsSummary![indexAll]]);
    }

    _favoritesPokemonsSummary.sort((a, b) => a.number.compareTo(b.number));

    _pokemonRepository
        .saveFavoritePokemonSummary(_favoritesPokemonsSummary.map((it) => it.number).toList());
  }

  @action
  void removeFavoritePokemon(String number) {
    final index = _favoritesPokemonsSummary.indexWhere((it) => it.number == number);

    if (index >= 0) {
      _favoritesPokemonsSummary.removeAt(index);
      _favoritesPokemonsSummary = ObservableList.of([..._favoritesPokemonsSummary]);
    }
  ...
  void addGenerationFilter(Generation generationFilter) {
    _pokemonFilter = _pokemonFilter.copyWith(generationFilter: generationFilter);
  }
  ...
  void setNameNumberFilter(String nameNumberFilter) {
    _pokemonFilter = _pokemonFilter.copyWith(pokemonNameNumberFilter: nameNumberFilter);
  }

  @action
  void clearNameNumberFilter() {
    _pokemonFilter = PokemonFilter(
        generationFilter: _pokemonFilter.generationFilter, typeFilter: _pokemonFilter.typeFilter);
  }
  ...
  Future<void> previousPokemon() async {
    final pokemonIndex = pokemonsSummary!.indexWhere((it) => it.number == _pokemon!.number);
  ...
  Future<void> _fetchFavoritesPokemons() async {
    ...
    _favoritesPokemonsSummary =
        ObservableList.of(_pokemonsSummary!.where((it) => favorites.contains(it.number)).toList());
  }
}
